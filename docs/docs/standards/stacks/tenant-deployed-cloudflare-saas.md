...
[Tenant-Deployed Cloudflare SaaS v1](/standards/tenant-deployed-cloudflare-saas/v1/) is the
judgeable rubric. Use it when a repository operates tenant-scoped Cloudflare deployment
surfaces.
...
|---|---|
| Repository | [vibecodeqa/ref-cloudflare-saas](https://github.com/vibecodeqa/ref-cloudflare-saas) (published) |
| VCQA report | [A 91/100](https://github.com/vibecodeqa/ref-cloudflare-saas/blob/main/docs/vcqa-report.md) |
| Verification | self-reported |
| Assessed commit | [`41eb243`](https://github.com/vibecodeqa/ref-cloudflare-saas/commit/41eb24369378b4efd4234d8ccea5e5e88b799761) |
| Assessed on | 2026-07-23 |
| CI run | [success](https://github.com/vibecodeqa/ref-cloudflare-saas/actions/runs/30010329406) (2026-07-23) |
...
- **Tenant isolation is judged from declared topology and its enforcement points.** A
  manifest plus per-tenant bindings is evidence of a model; it is not proof that no request
  can cross a tenant boundary.
- **Most of the surface lives outside the repository.** Cloudflare projects, custom
  domains, Access policies, and per-tenant secrets are configured in the platform, so the
  rubric asks for repo-visible declarations and CI evidence and falls back to evidence-only
  where a repo cannot show them.
- **The reference repo is deliberately richer than the minimum.** Do not read its React,
  MCP, SDK, or CLI packages as required composition; each is judged by its own standard.
- **Provisioning and deprovisioning are judged as runbooks.** VCQA can require the
  procedure to exist and name its evidence; it cannot verify it was followed for a given
  tenant.
- **Cost, quota, and noisy-neighbour behaviour are out of scope for v1**, even though they
  are real operational risks of tenant-per-deployment topologies.
...
| Maintenance | Value |
|---|---|
| Latest edition | [Tenant-Deployed Cloudflare SaaS v1](/standards/tenant-deployed-cloudflare-saas/v1/) |
| Pin reports and scans to | `/standards/tenant-deployed-cloudflare-saas/v1/` |
| Last reviewed | 2026-07 |
| Next review due | 2027-07 |
| Edition targets | `cloudflarePagesFunctions latest`, `cloudflareD1 latest`, `cloudflareWorkers latest`, `mcp 2025-11-25` |
| Lifecycle | active |
| Errata | none |
| Composes | `cloudflare-workers`, `github-actions`, `web-security`, `docs-kb` |
