...
Cloudflare deployable surfaces. This stack treats deployment topology, tenant
isolation, promotion, preview exposure, resource ownership, and operational evidence
...
- promotion gates, data-resource state, rollback, and fix-forward
...
|---|---|---|
| Example app/API surface | [`app/`](https://github.com/vibecodeqa/ref-cloudflare-saas/tree/main/app) | One deployable app surface shows static assets, same-origin API routes, tests, migrations, and deployment config together. |
| Example data migrations | [`app/migrations/`](https://github.com/vibecodeqa/ref-cloudflare-saas/tree/main/app/migrations) | Schema change history is versioned and can be applied in CI/deploy workflows for the tenant data resource used by the template. |
| Example Cloudflare bindings | [`app/wrangler.toml`](https://github.com/vibecodeqa/ref-cloudflare-saas/blob/main/app/wrangler.toml) | Cloudflare project, environment, and resource binding shape are explicit. |
| Example automation surface | [`packages/mcp-worker/`](https://github.com/vibecodeqa/ref-cloudflare-saas/tree/main/packages/mcp-worker) | Tenant operations can expose a remote Worker/MCP surface without mixing it into the frontend. |
| Example typed automation packages | [`packages/sdk/`](https://github.com/vibecodeqa/ref-cloudflare-saas/tree/main/packages/sdk), [`packages/cli/`](https://github.com/vibecodeqa/ref-cloudflare-saas/tree/main/packages/cli) | Contracted automation has typed package boundaries rather than ad hoc scripts. |
...
|---|---|
| White-label customer app | Each tenant has its own project, route, custom domain, Access policy, or environment binding. |
| Regulated tenant environment | Customer data, credentials, databases, storage resources, or deployment approvals are isolated per tenant or tenant environment. |
...
| Regional or data-residency tenant | Tenant placement affects Cloudflare resources, database/storage selection, secrets, domains, or observability. |
...
| Shared app with tenant deploy controls | The application artifact is shared, but tenant resource manifests, bindings, domains, secrets, databases, or storage resources vary by tenant. |
...
Choose this stack when each customer or customer environment needs its own deployable
Cloudflare surface: separate projects, Workers, routes/domains, databases, storage
resources, secrets, Access policies, or environment bindings. It fits white-label
SaaS, regulated customer deployments, tenant-specific release windows, regional data
placement, and products where a tenant incident must be contained without taking every
customer down.
...
share one deployable artifact, use lower-level Cloudflare app/API, data-resource, and
web-security standards instead.
...
| Need | Better fit |
|---|---|
| One production app serving every customer from shared deployables | Cloudflare app/API standards plus data-resource and web-security standards. |
| Row-level multi-tenancy inside one database and one deployment | Data-resource standard plus authorization and tenant data checks. |
...
D1 references apply when the tenant data resource is D1. Equivalent backup, restore,
migration, and isolation evidence is expected for KV, R2, Durable Objects, or external
database bindings used as tenant data resources.
...
- [Cloudflare Pages Functions](../items/cloudflare-pages-functions.md), when the tenant
  surface includes a Pages app or same-origin API.
- [Cloudflare D1](../items/cloudflare-d1.md), when tenant state uses D1.
- [Cloudflare Worker MCP Server](cloudflare-worker-mcp-server.md), when tenant admin or
  automation is exposed as a remote MCP server.
...
- Per-tenant data resource, migration, backup, and restore expectations.
...
- Rollback and fix-forward expectations for code, config, and data state.
...
- Data migration commands, tenant database/storage IDs, export/restore scripts, or Time
...
- Tenant secrets and data resources must not be shared across preview, staging, and
  production unless the repo documents the shared-infrastructure design and tests the
  authorization boundary.
- Promotion must move known artifacts forward: commit, Worker version, deployment,
  migration set, tenant manifest, and approval evidence.
...
- Data rollback is a product runbook, not just a platform feature. Code rollback and
  data restore/fix-forward need separate decisions and evidence.
...
- **R-DATA-1:** Tenant data-resource isolation is the default; D1-backed variants apply
  the D1-specific rubric.
...
- Treating tenant isolation as a naming convention while all tenants share one production
  database/storage resource, secret set, or integration credential.
- Letting PR previews use production secrets, production data bindings, production
...
- Promoting by rebuilding from a branch instead of referencing the tested commit,
  deployment/version, migration set, and tenant manifest.
- Assuming a code rollback also rolls back schema or data.
...
- Raises preview URLs, aliases, bindings, secrets, and data state to first-class
