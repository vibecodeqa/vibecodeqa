...
| Reference implementation | [vibecodeqa/ref-flutter-firebase-app](https://github.com/vibecodeqa/ref-flutter-firebase-app) (experimental) — see **Reference implementation** below. |
| Blocking promotion | Eight candidate rules are drafted, but none states a severity, required evidence, or accepted exception, so no rubric can be cut from them. `ref-flutter-firebase-app` is now built and cataloged as experimental with a self-reported score and a green CI run; the v1 rubric half of [#43](https://github.com/vibecodeqa/vibecodeqa/issues/43) is still outstanding and is what blocks a candidate rubric. Detection is also fragile: it requires `melos.yaml`, which Melos 7 removed ([#48](https://github.com/vibecodeqa/vibecodeqa/issues/48)). |
...
- `ref-flutter-firebase-app` is the product-neutral template for this composition; it is built
  and cataloged as experimental, and its evidence is in **Reference implementation** above.
