...
| Reference implementation | Not built. `ref-flutter-firebase-app` is a catalog candidate, not an existing repository, so no rule on this page has been proven against one. |
| Blocking promotion | Eight candidate rules are drafted, but none states a severity, required evidence, or accepted exception, so no rubric can be cut from them. Maturing the rules and building `ref-flutter-firebase-app` is tracked by [#43](https://github.com/vibecodeqa/vibecodeqa/issues/43). |
