...
| Reference implementation | Not built. `ref-typescript-sdk` is a catalog candidate, not an existing repository, so no rule on this page has been proven against one. |
| Blocking promotion | Twelve candidate rules carry severity, evidence, and exception terms. Blocking v1: `ref-typescript-sdk` does not exist yet ([#14](https://github.com/vibecodeqa/vibecodeqa/issues/14)), so not one rule has been proven against a working repository. |
...
2. `ref-typescript-sdk` exists and demonstrates the rules this charter claims are checkable,
   with score evidence in `compositions.json`
   ([#14](https://github.com/vibecodeqa/vibecodeqa/issues/14)). This is the binding gap: not
   one rule on this page has been judged against a working repository, so the rules are
